import { View, Text, Button, SafeAreaView, Pressable, Keyboard, KeyboardAvoidingView, Dimensions, Alert } from 'react-native'
...
import { SET_URL_TIKTOK ,INSTALL} from './../../../assets/redux/actions/authActions';
const index = ({SET_URL_TIKTOK,INSTALL,url_tiktok,loading}:{INSTALL:any,SET_URL_TIKTOK:any,url_tiktok:string,loading:boolean}) => {
        ...
        <SafeAreaView>
            ...
            <View className='flex flex-col relative  w-full h-full items-center justify-start bg-[#000000] overflow-hidden  '>
                ...
                onPress={()=>{
                    ...
                    INSTALL(urlss);
                    async () =>
                        await analytics().logEvent('install', {
                            id: 3745092,
                            item: 'tiktok install ',
                            description: [`${urlss}`],
                            size: 'L',
...
const mapStateToProps = (state:{url:{url_tiktok:string,loading:boolean}}) => {
    return {
        url_tiktok: state.url.url_tiktok,
        loading: state.url.loading,
        

    };
};  
export default connect(mapStateToProps,{SET_URL_TIKTOK,INSTALL})(index);